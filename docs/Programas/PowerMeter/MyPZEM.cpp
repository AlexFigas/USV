...
bool PZEM::updateValues() {
  ...
  if( this->receive(response, 25) != 25) { // Something went wrong
	  return false;
  }
  ...
  this->currentValues.energy =        ((uint32_t)response[13] << 8 | // Raw Energy in 1Wh
...
PZEM::PZEM(HardwareSerial* port, uint8_t receivePin, uint8_t transmitPin, uint8_t addr) {
  DebugMessagePrintf( "PZEM::PZEM(HardwarePort, receivePin=%d, transmitPin=%d, addr=0x%X)\n", receivePin, transmitPin, addr);
...
void PZEM::search() {
  ...
  for(uint16_t addr = 0x01; addr <= 0xF8; addr++) {
  	...
  	if ( this->receive(response, 7) != 7) { // Something went wrong
  	  continue;
  	}
  	else {
