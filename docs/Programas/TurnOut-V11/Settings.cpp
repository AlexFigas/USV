...
{
    DebugMessagePrintf(
        "{Units: %s, Min value: %d, Value: %d, Max value: %d}\n", this->units, this->min, this->value, this->max);
}
...
void SettingsServo::show()
{
    ...
    DebugMessagePrintf("Name: %s\nDescription: %s\nExpander: %d\nIndex: %d\nSmooth movement=%s",
                       ...
                       this->expander,
                       this->index,
                       this->smooth == true ? "true" : "false");
    DebugMessagePrintf("Min position: ");
    this->minPosition.show();
    DebugMessagePrintf("Max position: ");
    this->minPosition.show();
    DebugMessagePrintf("}\n");
}
...
static char* readElement(XMLElement* parent, const char* elemName)
{
    ...
    // DebugMessagePrintf( "XMLElement(%s) value %s present (%s).\n", elemName, auxValue!=NULL ? "is" : "isn't",
    // auxValue!=NULL ? auxValue : "N.A." );
...
static char* readAttribute(XMLElement* elem, const char* attributeName)
{
    ...
    // auxValue!=NULL ? "is" : "isn't", auxValue!=NULL ? auxValue : "N.A." );
...
{
    for (XMLElement* elemPort = elemPorts->FirstChildElement("PortIn"); elemPort != NULL;
         elemPort = elemPort->NextSiblingElement())
    {
        if (elemPort != NULL)
        {
...
{
    for (XMLElement* elemPort = elemPorts->FirstChildElement("PortOut"); elemPort != NULL;
         elemPort = elemPort->NextSiblingElement())
    {
        if (elemPort != NULL)
        {
...
{
    for (XMLElement* elemFence = elemFences->FirstChildElement("Fence"); elemFence != NULL;
         elemFence = elemFence->NextSiblingElement())
    {
        if (elemFence != NULL)
        {
...
static void writeSettings(PtrSettingsWiFi settingsWifi,
                          ...
                          PtrSettingsCrossings settingsCrossings)
{
    ...
    DebugMessagePrintf(
        "Settings::writeSettings(PtrSettingsWiFi, PtrSettingsRange, PtrSettingsExpandersPWM, PtrSettingsExpandersIO, "
        "PtrSettingsServos, PtrSettingsCrossings)\n");
...
void readSettings()
{
    ...
    XMLElement* elemSettingsGobMot =
        doc.FirstChildElement("Settings")->FirstChildElement("GlobalMotion")->FirstChildElement("Range");
    ...
    for (XMLElement* elemExpander = elemSettingsExpandersPWM->FirstChildElement(); elemExpander != NULL;
         elemExpander = elemExpander->NextSiblingElement())
    {
    ...
    for (XMLElement* elemExpander = elemSettingsExpandersIO->FirstChildElement(); elemExpander != NULL;
         elemExpander = elemExpander->NextSiblingElement())
    {
        if (elemExpander != NULL)
        {
    ...
    for (XMLElement* elemServo = elemSettingsServos->FirstChildElement(); elemServo != NULL;
         elemServo = elemServo->NextSiblingElement())
    {
        if (elemServo != NULL)
        {
    ...
    for (XMLElement* elemCrossing = elemSettingsCrossings->FirstChildElement(); elemCrossing != NULL;
         elemCrossing = elemCrossing->NextSiblingElement())
    {
