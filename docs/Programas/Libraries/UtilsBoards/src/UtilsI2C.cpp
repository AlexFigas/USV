...
void scanBus(TwoWire& bus, Print* pr, char* busName, bool showInSerial) {
	...
	if ( showInSerial ) Serial.printf( "I2C(%s) scan...\n", busName!=NULL ? busName : "Wire" );
	...
	for(byte address = 1, error; address < 127; ++address ) {
		...
		switch ( error ) {
			case 0:
				++nDevices;
				pr->printf( "I2C 0x%2X\n", address );
				if ( showInSerial ) Serial.printf( "I2C 0x%2X\n", address );
				break;

			case 4:
				pr->printf( "Error 0x%2X\n", address );
				if ( showInSerial ) Serial.printf( "Error 0x%2X\n", address );
				break;
		}
	}
	...
	if ( nDevices==0 ) {
		...
		if ( showInSerial ) Serial.println( "No I2C devs." );
	}

	pr->println( "I2C scan done." );
	if ( showInSerial ) Serial.println( "I2C scan done." );
}